...
from PyQt5.QtWidgets import (QApplication, QWidget, QTableWidget, QVBoxLayout, QPushButton, QFileDialog, QLabel, QComboBox, QTextEdit,
                             ...
                             QFrame, QSizePolicy, QToolButton, QTabWidget, QMenu, QAction, QTableWidgetItem)
...
class CAFilter:
    def __init__(self):
        self.Sf = np.zeros((9, 1))  # Filter state vector
        self.Pf = np.eye(9)  # Filter state covariance matrix
        self.Sp = np.zeros((9, 1))  # Predicted state vector
        self.Pp = np.eye(9)  # Predicted state covariance matrix
        self.plant_noise = 20  # Plant noise covariance
    ...
    def initialize_filter_state(self, x, y, z, ax, ay, az, time):
        if not self.first_rep_flag:
            self.Z1 = np.array([[x], [y], [z]])
            self.Sf[0] = x
            self.Sf[1] = y
            ...
            self.Meas_Time = time
            dt = self.Meas_Time - self.prev_Time
            self.ax = (self.Z1[0] - self.Z2[0]) / (dt * dt)
            self.ay = (self.Z1[1] - self.Z2[1]) / (dt * dt)
            self.az = (self.Z1[2] - self.Z2[2]) / (dt * dt)
            self.Meas_Time = time
            self.second_rep_flag = True
        else:
            self.Z = np.array([[x], [y], [z]])
    ...
    def update_step(self, Z):
        ...
        K = np.dot(np.dot(self.Pp, self.H.T), np.linalg.inv(S))
        self.Sf = self.Sp + np.dot(K, Inn)
        self.Pf = np.dot(np.eye(9) - np.dot(K, self.H), self.Pp)


class CTFilter:  # Add CT Filter class
    def __init__(self):
        self.Sf = np.zeros((12, 1))  # Filter state vector
        self.Pf = np.eye(12)  # Filter state covariance matrix
        self.Sp = np.zeros((12, 1))  # Predicted state vector
        self.Pp = np.eye(12)  # Predicted state covariance matrix
        self.plant_noise = 20  # Plant noise covariance
        self.H = np.eye(3, 12)  # Measurement matrix
        self.R = np.eye(3)  # Measurement noise covariance
        self.Meas_Time = 0  # Measured time
        self.prev_Time = 0
        self.Q = np.eye(12)
    ...
    def initialize_filter_state(self, x, y, z, vx, vy, vz, ax, ay, az, time):
        if not self.first_rep_flag:
            self.Z1 = np.array([[x], [y], [z]])
            self.Sf[0] = x
            self.Sf[1] = y
    ...
    def update_step(self, Z):
        ...
        K = np.dot(np.dot(self.Pp, self.H.T), np.linalg.inv(S))
        self.Sf = self.Sp + np.dot(K, Inn)
        self.Pf = np.dot(np.eye(12) - np.dot(K, self.H), self.Pp)
...
def main(input_file, track_mode, filter_option, association_type):
    ...
    progression_states = {
        ...
        7: ['Poss1', 'Poss2', 'Poss3', 'Tentative1', 'Tentative2', 'Tentative3', 'Firm']
    }[firm_threshold]
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        if len(group) == 1:  # Single measurement
            ...
            if association_method == 'JPDA':
                ...
                # Log clusters for JPDA
                log_data = {
                    'cluster Generated': str(clusters),
                }
                ...
                # Log cost matrix for Munkres
                log_data = {
                    'cost_matrix': str(best_reports),
                }
        ...
        for track_id, track in enumerate(tracks):
            ...
            # Check if the track should transition to 'Firm'
            if hit_counts[track_id] >= firm_threshold and current_state != 'Firm':
                ...
                # Transition to the next state if conditions are met
            ...
            # Update the track's current state
...
class KalmanFilterGUI(QWidget):
    ...
    def update_plot(self):
        ...
        def on_add(sel):
            ...
            plant_noise = self.tracks[index // len(self.tracks[0]['measurements'])]['Pf'][0, 0]
    ...
    def plot_all_modes(self, tracks, ax):
        ...
        # Plot Range vs Time
        plot_measurements(tracks, axes[0, 0], "Range vs Time")
        axes[0, 0].set_title("Range vs Time")

        # Plot Azimuth vs Time
        plot_measurements(tracks, axes[0, 1], "Azimuth vs Time")
